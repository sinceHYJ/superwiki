...
import type { ConflictChoice, SyncConflict, SyncEntry, SyncManifest, SyncOperation, SyncProvider, SyncSnapshot, SyncStatus } from "./types";
...
export class SyncController {
  ...
  private async perform(root: string, settings: WorkspaceSyncSettings, choices: Record<string, ConflictChoice> | undefined) {
    ...
    try {
      ...
      this.update({ activities: localResult.skipped.map((path) => ({ action: "skip", path, message: "已跳过符号链接或特殊文件" })) });
      ...
      for (const operation of operations) {
        ...
        const current = await scanLocalWorkspace(root);
        const currentGlobal = globalSnapshot(current.snapshot, settings.relativeScope);
        baselineState = { ...baselineState, ...filterSnapshot(manifest.entries, settings.relativeScope), ...currentGlobal };
        await saveBaseline(settings.bindingId, baselineState);
        completed += 1;
      ...
      manifest.revision += 1;
      await provider.saveManifest(manifest, lease);
      await saveBaseline(settings.bindingId, { ...baselineState, ...filterSnapshot(manifest.entries, settings.relativeScope), ...finalLocal });
      this.retryIndex = 0;
  ...
  private async preserveBoth(root: string, settings: WorkspaceSyncSettings, provider: SyncProvider, manifest: SyncManifest, lease: NonNullable<Awaited<ReturnType<SyncProvider["acquireLease"]>>>, conflict: SyncConflict, deviceId: string) {
    if (conflict.local && conflict.local.kind !== "deleted") {
      ...
      manifest.revision += 1;
      await provider.saveManifest(manifest, lease);
      this.status.activities.push({ action: "conflict", path: copyPath, message: "已保留本地冲突副本" });
    }
  ...
  private async executeOperation(root: string, settings: WorkspaceSyncSettings, provider: SyncProvider, operation: SyncOperation, progress?: (completed: number, total: number) => void) {
    ...
    switch (operation.kind) {
      case "upload": {
        ...
        this.status.activities.push({ action: "upload", path: operation.path, message: "已上传" });
        break;
      }
      case "download":
        await provider.download(operation.path, await localWriter(root, path), this.abortController!.signal, progress);
        this.status.activities.push({ action: "download", path: operation.path, message: "已下载" });
        break;
      case "createLocalDirectory":
        await createLocalDirectory(root, path);
        this.status.activities.push({ action: "directory", path: operation.path, message: "已创建本地文件夹" });
        break;
      case "createRemoteDirectory":
        await provider.createDirectory(operation.path);
        this.status.activities.push({ action: "directory", path: operation.path, message: "已创建云端文件夹" });
        break;
      case "deleteLocal":
        await deleteLocalEntry(root, path);
        this.status.activities.push({ action: "delete", path: operation.path, message: "已删除本地内容" });
        break;
      case "deleteRemote":
        await provider.remove(operation.path, operation.entry.revision);
        this.status.activities.push({ action: "delete", path: operation.path, message: "已删除云端内容" });
        break;
      case "confirm":
        break;
    }
